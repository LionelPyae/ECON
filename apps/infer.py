...
if __name__ == "__main__":
    ...
    for data in pbar:
        ...
        smpl_path = f"{args.out_dir}/{cfg.name}/obj/{file_name}_smpl_00.obj"
        ...
        if osp.exists(smpl_path):
            ...
            for idx in range(N_body):
                ...
                smpl_obj = f"{args.out_dir}/{cfg.name}/obj/{file_name}_smpl_{idx:02d}.obj"
            ...
            if not args.novis:
                ...
                per_data_lst[-1].save(os.path.join(file_path, f"{file_name}_smpl.png"))
                # per_data_lst[-1].save(
                #     osp.join(file_path, f"{data['name']}_smpl.png")
                # )
        ...
        if not args.novis:
            ...
            img_overlap_path = osp.join(args.out_dir, cfg.name, f"png/{file_name}_overlap.png")
            torchvision.utils.save_image(
        ...
        for idx in range(N_body):
            ...
            final_path = f"{args.out_dir}/{cfg.name}/obj/{file_name}_{idx}_full.obj"
            ...
            BNI_dict = save_normal_tensor(
                in_tensor,
                idx,
                osp.join(args.out_dir, cfg.name, f"BNI/{file_name}_{idx}"),
                cfg.bni.thickness,
            )
            ...
            if cfg.bni.use_ifnet:
                ...
                side_mesh_path = f"{args.out_dir}/{cfg.name}/obj/{file_name}_{idx}_IF.obj"
            ...
            BNI_object.F_B_trimesh.export(
                f"{args.out_dir}/{cfg.name}/obj/{file_name}_{idx}_BNI.obj"
            )
            full_lst = []
            ...
            if "face" in cfg.bni.use_smpl:
                ...
                face_mesh.export(f"{args.out_dir}/{cfg.name}/obj/{file_name}_{idx}_face.obj")
                full_lst += [face_mesh]
            ...
            if "hand" in cfg.bni.use_smpl and (True in data['hands_visibility'][idx]):
                ...
                hand_mesh.export(f"{args.out_dir}/{cfg.name}/obj/{file_name}_{idx}_hand.obj")
                full_lst += [hand_mesh]
            ...
            side_mesh.export(f"{args.out_dir}/{cfg.name}/obj/{file_name}_{idx}_side.obj")
        ...
        if len(per_loop_lst) > 0 and (not args.novis):
            ...
            per_data_lst[-1].save(osp.join(args.out_dir, cfg.name, f"png/{file_name}_cloth.png"))
            ...
            torch.save(
                in_tensor, osp.join(args.out_dir, cfg.name, f"vid/{file_name}_in_tensor.pt")
            )
